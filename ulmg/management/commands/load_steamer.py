...
class Command(BaseCommand):
    ...
    def get_pitchers(self):
        with open('data/2020/steamer_pitch.csv', 'r') as readfile:
            players = [dict(p) for p in csv.DictReader(readfile) if int(p['IP'].split(".")[0]) >= 5]
            ...
            for p in players:
                ...
                if len(obj) == 1:
                    ...
                    obj.ps_is_mlb = True
                    obj.ps_g = int(p['G'])
                    ps_era = Decimal(p['ERA'])
                    ps_fip = Decimal(p['FIP'])
                    ps_gs = int(p['GS'])
                    ps_sv = int(p['SV'])
                    ps_ip = Decimal(p['IP'])
                    ps_h = int(p['H'])
                    ...
                    ps_so = int(p['SO'])
                    ps_bb = int(p['BB'])
                    ps_whip = Decimal(p['WHIP'])
                    ps_k_9 = Decimal(p['K/9'])
                    ps_bb_9 = Decimal(p['BB/9'])
                    ps_ra9_war = Decimal(p['RA9-WAR'])
                    obj.ps_war = Decimal(p['WAR'])
                    obj.save()