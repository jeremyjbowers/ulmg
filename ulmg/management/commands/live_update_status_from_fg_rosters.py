...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write("Cleared existing roster status fields")
        except Exception as e:
            self.stderr.write(f"ERROR clearing roster status fields: {e}")
            results['failed'].append(f"Clear roster status: {e}")

        for team_id, team_abbrev, team_name in teams:
            try:
                self.stdout.write(f"Processing {team_name} ({team_abbrev})...")
                ...
                if not roster:
                    ...
                    results['failed'].append(f"{team_name}: No roster data found")
                    continue
                ...
                for player in roster:
                    try:
                        ...
                        self.stderr.write(f"ERROR processing player {player_name} for {team_name}: {e}")
                        results['players_failed'] += 1
                        # Continue with next player
                        continue

                results['success'].append(f"{team_name}: {team_players_processed} players processed")
                self.stdout.write(f"✓ {team_name}: {team_players_processed} players processed")
                
            except Exception as e:
                self.stderr.write(f"✗ ERROR processing team {team_name}: {e}")
                results['failed'].append(f"{team_name}: {str(e)}")
                continue

        # Print summary
        self.stdout.write('\n' + '='*50)
        self.stdout.write('ROSTER STATUS UPDATE SUMMARY')
        self.stdout.write('='*50)
        self.stdout.write(f'Players processed: {results["players_processed"]}')
        self.stdout.write(f'Players failed: {results["players_failed"]}')
